...
export class AdminEmailController {
    ...
    static async sendIosLaunch(req: Request, res: Response) {
        try {
            const { target, testEmail } = req.body;
            ...
            if (target === 'test') {
                ...
                console.log(`[AdminEmail] Sending test iOS launch email to ${emails.length} recipients individually...`);
                await Promise.all(
                    emails.map(email => EmailService.sendIosLaunchNotificationEmail(email))
                );
                return res.json({ success: true, message: `Test iOS email sent to ${emails.length} recipients individually` });
            }
            ...
            if (target === 'all') {
                ...
                const emails = rows.map(r => r.email).filter(Boolean);
                console.log(`[AdminEmail] Starting one-by-one broadcast to ${emails.length} waitlisted users for iOS launch...`);

                AdminEmailController.broadcastIosEmails(emails).catch(err => {
                    console.error('[AdminEmail] Background broadcast failed:', err);
                });

                return res.json({
                    success: true,
                    message: `Started background broadcast of iOS launch email to ${emails.length} users. Check logs for details.`
                });
            }

            return res.status(400).json({ error: 'Invalid target. Must be "test" or "all"' });
        } catch (error: any) {
            console.error('[AdminEmail] sendIosLaunch failed:', error);
            return res.status(500).json({ error: error.message || 'Internal server error' });
        }
    }

    private static async broadcastIosEmails(emails: string[]) {
        const batchSize = 10;
        const delayMs = 1000;

        for (let i = 0; i < emails.length; i += batchSize) {
            const batch = emails.slice(i, i + batchSize);
            console.log(`[AdminEmail] Sending iOS batch ${Math.floor(i / batchSize) + 1} (${batch.length} emails, progress: ${i}/${emails.length})...`);
            
            await Promise.all(
                batch.map(async (email) => {
                    try {
                        await EmailService.sendIosLaunchNotificationEmail(email);
                        console.log(`[AdminEmail] Successfully sent iOS email to ${email}`);
                    } catch (err) {
                        console.error(`[AdminEmail] Failed to send iOS email to ${email}:`, err);
                    }
                })
            );
        ...
        console.log(`[AdminEmail] iOS Broadcast complete. Processed ${emails.length} emails.`);
    }
}
